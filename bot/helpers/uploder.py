...
from .utils import *
from config import Config   # root config.py ကို import
...
async def telegram_upload(track, user):
    """
    Upload single track to both User & Channel with full metadata (art, title, artist, duration).
    """
    # Upload to user (with reply_id for context)
    await send_message(user, track['filepath'], 'audio', meta=track)

    # Upload to channel (NO reply_id)
    if Config.CHANNEL_ID:
        channel_user = user.copy()
        channel_user['r_id'] = None   # prevent reply_to_message_id in channel
...
async def batch_telegram_upload(metadata, user):
    """
    Upload albums/playlists/artists tracks one by one to User & Channel.
    """
